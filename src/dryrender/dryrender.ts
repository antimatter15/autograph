...
export default function dryRender(node: React.ReactNode, fiber: ReactFiber | null): void {
    if(Array.isArray(node)){
        ...
        return dryRender(React.createElement((node as JSX.Element).type.type, (node as JSX.Element).props), fiber && fiber.child)
        ...
        if((node as JSX.Element).type._status === 1){
            return dryRender(React.createElement((node as JSX.Element).type._result, (node as JSX.Element).props), fiber)
        }else{
    ...
    if(fiber && !(
        fiber.type === (node as JSX.Element).type
    )){
    ...
    if(typeof (node as JSX.Element).type === 'string'){
    ...
    }else if(typeof (node as JSX.Element).type === 'function' && (node as JSX.Element).type.prototype && (node as JSX.Element).type.prototype.isReactComponent){
        console.log('Class component', (node as JSX.Element).type.name)
        // https://overreacted.io/how-does-react-tell-a-class-from-a-function/
        let stateNode = new (node as JSX.Element).type((node as JSX.Element).props, /* TODO: legacy context API */);
        stateNode.props = (node as JSX.Element).props;
        if(fiber) stateNode.state = readLatestClassComponentState(fiber, stateNode, (node as JSX.Element).props);
        ...
        if(ALLOW_LEGACY_CONTEXT_API && (node as JSX.Element).type.contextTypes) legacyContext = readLegacyContext(fiber, (node as JSX.Element).type.contextTypes);
        ...
        let nextChildren = (node as JSX.Element).type.render((node as JSX.Element).props, (node as any).ref);
        dryRender(nextChildren, fiber && fiber.child)
    }else{
