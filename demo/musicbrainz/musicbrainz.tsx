...
function App(){
    ...
    return <div>{query.search!.artists({ query: 'metric', first: 10 })!.edges!.map(edge => {
        let artist = edge.node!
        return <div>
            <h3>{artist.name}</h3>
            {artist.rating!.voteCount > 0 && <div>{artist.rating!.value} ({artist.rating!.voteCount} votes)</div>}
            <ul>{artist.works({ first: 10 })!.nodes!.map(work => {
